...
public class Bullet : MonoBehaviour
{
    ...
    void OnTriggerEnter2D(Collider2D t_other)
    {
        ...
        {
            if (t_other.gameObject.tag == "Bomb") { Destroy(t_other.gameObject); SoundManagerScript.PlaySound("bulletHit"); }
            else if (t_other.gameObject.tag == "Shrapnel") { Destroy(t_other.gameObject); SoundManagerScript.PlaySound("bulletHit"); }
            else if (t_other.gameObject.tag == "Bomber") { t_other.gameObject.GetComponent<Bomber>().Damage(1); SoundManagerScript.PlaySound("bulletHit"); }
            else if (t_other.gameObject.tag == "Follower" && !t_other.gameObject.GetComponent<FlyingFollower>().invincible) { t_other.gameObject.GetComponent<FlyingFollower>().damage(1); SoundManagerScript.PlaySound("bulletHit"); }
